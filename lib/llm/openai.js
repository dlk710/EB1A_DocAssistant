...
import { CLASSIFICATION_SCHEMA, PROMPT_REVIEW_SCHEMA, buildUserPrompt, resolvePrompts } from './index.js';
...
export class OpenAIProvider {
  ...
  async reviewPromptSections(sections) {
    const candidate = resolvePrompts({ sections });
    let completion;
    try {
      completion = await this.client.chat.completions.create({
        model: this.model,
        messages: [
          {
            role: 'system',
            content: [
              'You review prompt libraries for an EB1A evidence classification app.',
              'Check for contradictions, unclear instructions, missing guardrails, duplicated logic, category leakage, and mismatches between event-first behavior and section-specific rules.',
              'Be strict. Only approve green/save_ready=true when the prompt set is coherent enough to use as the live master prompt without obvious risk.',
              'Prefer short, concrete findings.',
            ].join(' '),
          },
          {
            role: 'user',
            content: [
              'Review these prompt sections as one prompt library.',
              'Return whether the set is ready to save as the final master prompt.',
              '',
              `Sections JSON:\n${JSON.stringify(sections, null, 2)}`,
              '',
              `Assembled system prompt:\n${candidate.systemPrompt}`,
              '',
              `Assembled user prompt template:\n${candidate.userPromptTemplate}`,
            ].join('\n'),
          },
        ],
        response_format: {
    ...
    const raw = completion.choices[0]?.message?.content;
    if (!raw) throw new Error('OpenAI returned no prompt review content.');
    const review = JSON.parse(raw);
    return {
      ...review,
      reviewed_at: new Date().toISOString(),
      master_system_prompt: candidate.systemPrompt,
      master_user_prompt: candidate.userPromptTemplate,
    };
  }
  ...
  async classifyBatch(items, { onProgress } = {}) {
    ...
    const activeItems = items
      .map((item, idx) => item ? { ...item, __idx: idx } : null)
      .filter(Boolean);
    if (activeItems.length === 0) {
      return new Array(items.length).fill(null);
    }
    try {
      const lines = activeItems.map((item) => JSON.stringify({
        custom_id: String(item.__idx),
        method: 'POST',
      ...
      emit({ phase: 'batch', message: `Uploading batch payload for ${activeItems.length} files…`, done: 0, total: items.length });
      ...
      const batch = await this.client.batches.create({
        ...
        metadata: {
          app: 'eb1a-organizer',
          model: this.model,
          file_count: String(activeItems.length),
        },
      });
      ...
      const completed = await this.waitForBatch(batch.id, activeItems.length, emit);
  ...
  async readBatchResults(batch, items) {
    ...
    return parsed.map((entry, idx) => {
      ...
      return entry || { error: new Error(`Missing batch result for ${items[idx].filename}`) };
    });
  }
}
