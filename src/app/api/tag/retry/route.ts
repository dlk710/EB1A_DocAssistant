import { startWorkspaceCriteriaTaggingJob } from "@/lib/criteria-tagging";
...
export async function POST(request: Request) {
  const payload = (await request.json()) as { workspaceId?: string; docIds?: string[] } | null;
  ...
  if (!eventBundles || eventBundles.status !== "completed" || !classification || classification.status !== "completed") {
    return Response.json({ ok: false, error: { code: "not_ready", message: "Tagging retry is only available after bundling and classification complete." } }, { status: 400 });
  }
  ...
  return Response.json({ ok: true, data: { jobId: job.id } });
}
