...
export async function runPipeline({ inputDir, outputDir, llm, settings, onProgress }) {
  ...
  for (const { file, result } of classified) {
    ...
    audit.push({
      ...
      type: result.criterion === '_Unclassified' ? 'Unclassified' : 'Classified',
      hash: file.hash,
...
async function classifyInBatch(prepared, llm, emit, context = {}) {
  const outputs = await llm.classifyBatch(
    prepared.map(item => ({ filename: item.file.name, text: item.text, context: { extraContext: [context.criteriaContextText, context.projectContextText].filter(Boolean).join('\n\n') } })),
...
function buildSummary({ files, classified, referenceFiles, duplicates, audit, llm, projectRegistry, selectedCriteria }) {
  ...
  const criteriaWithEvidence = Object.keys(buckets).filter(k => !k.startsWith('_')).length;
