...
const CRITERION_DISPLAY = {
  ...
  'CLEANUP':                      { name: 'CLEANUP',                      cite: 'System cleanup bucket' },
};
...
export async function writeIndexFiles({ outputDir, inputDir, classified, referenceFiles, duplicates, llm, projectRegistry, selectedCriteria }) {
  ...
  for (const c of classified) {
    if (c.result.criterion === '_Unclassified' || c.result.criterion === 'CLEANUP') continue;
  ...
  lines.push(`| Classified into criteria | ${classified.filter(c => c.result.criterion !== '_Unclassified' && c.result.criterion !== 'CLEANUP').length} |`);
  lines.push(`| Cleanup by filename      | ${classified.filter(c => c.result.criterion === 'CLEANUP').length} |`);
  ...
  const cleanup = classified.filter(c => c.result.criterion === 'CLEANUP');
  if (cleanup.length > 0) {
    lines.push('## CLEANUP');
    lines.push(`*${cleanup.length} file${cleanup.length === 1 ? '' : 's'} routed here because the filename contains REMOVE or DELETE*`);
    lines.push('');
    for (const c of cleanup) {
      lines.push(`- [${c.file.name}](./CLEANUP/${c.file.name}) — ${c.result.reason}`);
    }
    lines.push('');
  }
